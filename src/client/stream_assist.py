"""StreamAssist client for Discovery Engine REST API.

Ported from github.com/jswortz/hello_gemini_agent with config-driven settings,
type hints, and response parsing helpers.
"""
...
class StreamAssistClient:
    ...
    Handles authentication, session management, retry logic, and
    connection pooling for querying Gemini Enterprise agents.
    """
    ...
    @classmethod
    def from_config(cls, config_path: Optional[Path] = None) -> "StreamAssistClient":
    ...
    def create_session(self, display_name: str = "WorkshopSession") -> str:
        """Create a conversation session. Returns the full session resource name."""
    ...
    def query(
        ...
        Args:
            ...
            session_id: Optional session resource name. Uses auto-session if omitted.
